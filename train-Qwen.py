...
def dataset_jsonl_transfer(origin_path, new_path):
    ...
    with open(origin_path, "r",encoding='utf-8') as file:
        for line in file:
            ...
            idid=data["id"]
            context = data["original_text"]
            catagory = data["wrong_ids"]
            label = data["correct_text"]
            message = {
...
def process_func(example):
    ...
    instruction = tokenizer(
        f"<|im_start|>system\n你是一个文本文本校对领域的专家，你会接收到一段文本，请查找可能存在的错误，并输出正确的文本内容<|im_end|>\n<|im_start|>user\n{example['input']}<|im_end|>\n<|im_start|>assistant\n",
...
args = TrainingArguments(
    output_dir="/root/autodl-tmp/output/Qwen2",
